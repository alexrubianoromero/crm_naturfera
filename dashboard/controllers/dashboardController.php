...
class dashboardController
{
    ...
    public function __construct()
    {
        ...
        if($_REQUEST['opcion']=='tablaResultadosOrdenes')
        {
                ...
                $this->view->tablaResultadosOrdenes($clientes);

        }
        if($_REQUEST['opcion']=='tablaResultadosOrdenesFiltroNombre')
        {
                $clientes =    $this->clienteModel->traerClientesFiltroNombre($_REQUEST['nombre']); 
       
                $this->view->tablaResultadosOrdenes($clientes);

        }
        if($_REQUEST['opcion']=='tablaResultadosOrdenesFiltroEmpresa')
        {
                $clientes =    $this->clienteModel->traerClientesFiltroEmpresa($_REQUEST['empresa']); 
       
                $this->view->tablaResultadosOrdenes($clientes);

        }
        if($_REQUEST['opcion']=='contenidoDetalleContacto')
        {
